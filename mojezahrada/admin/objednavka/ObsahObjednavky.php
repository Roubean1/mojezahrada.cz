...
<head>
    ...
    ?>
    <link rel="stylesheet" href="../../styles/ObsahObjednavky-style.css">
</head>
<body>
<?php
...
$vysledek = $db->vybrat($tabulka,$dotaz,$podminka);
    ?>
    <button><a href="ObsahObjednavkyPridat.php?id_o=<?=$_GET["id_o"]?>">Přidat objednavku</a></button>
    <h1>Obsah objednávky č.<?=$_GET["id_o"]?>:</h1>
<?php
if ($vysledek->num_rows > 0){
    ?>
    <table class="vypisObsahuObjednavky" cellspacing="0">
        <thead>
        <tr>
        ...
        while ($radek = $vysledek->fetch_assoc()) {
            ...
            <tr>
                ...
                <td><?= $radek['cena']; ?> Kč</td>
                <td>
                    <a href="ObsahObjednavkyEditace.php?id_o=<?= $_GET['id_o']; ?>&id_p=<?= $radek['id_p'];?>">Editovat</a>
                    <a href="ObsahObjednavkySmazat.php?id_o=<?= $_GET['id_o']; ?>&id_p=<?= $radek['id_p'];?>">Smazat</a>
                </td>
            </tr>
            <?php
        }
